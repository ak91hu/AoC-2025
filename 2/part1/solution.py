...
# Iterate through possible "seeds" to generate invalid IDs.
# The max value in the ranges is ~6.6 billion (10 digits), 
# so we only need to check seeds up to 5 digits (1 to 99999).
for seed in range(1, 100000):
    # Construct the invalid ID by repeating the seed
    candidate_str = str(seed) + str(seed)
    candidate = int(candidate_str)
    
    # Check if this candidate exists in any of the provided ranges
...
print(f"Number of invalid IDs found: {len(found_ids)}")
