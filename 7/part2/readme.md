...
Both parts function by scanning the grid row by row, simulating the downward movement of the tachyon from the starting point S. In Part 1, the simulation tracks the physical presence of beams using a Set, which only records where a beam is, ignoring how many paths led there. When beams hit a splitter in Part 1, they branch left and right, but if multiple branches land on the same tile, they merge into a single beam. This means Part 1 is simply calculating the geometric "spread" of the beam to count how many unique splitters are physically hit. Part 2 changes the rules to track distinct "timelines," using a Dictionary to count exactly how many theoretical particles exist at each column index. Under these Quantum rules, when a particle hits a splitter, it effectively duplicates itself, sending its current count to both the left and right paths simultaneously. The key difference is that while Part 1 discards overlapping paths (merging them into one), Part 2 adds their counts together, preserving the history of every single split.
